# Feature Specification: MK3 Realtime Viewer
...
**Input**: User description: "Profesjonalny realtime viewer do projektu MK3 oraz ustawień
peryferiów przedstawionych w sposób czytelny dla człowieka, oparty na STM32G0B0 i istniejącej
mapie rejestrów, bez zapisów lub kasowania w procesorze."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Observe MK3 Live (Priority: P1)

An engineer connects an authorized MK3 through ST-Link and immediately sees a coherent live
overview of board state, target identity, connection health, update rate, and stale-data status.
...
**Independent Test**: Connect a running MK3, observe the dashboard for 30 minutes, and verify that
values update, stale or disconnected data is marked promptly, and an independent target audit finds
no write, erase, reset, halt, run, or step request.

**Acceptance Scenarios**:

1. **Given** a supported powered MK3 and an authorized programmer, **When** the user starts live
   observation, **Then** the viewer shows target identity, supply voltage, freshness, sample rate,
   and the latest coherent project snapshot within three seconds.
...
An engineer selects an MK3 subsystem or MCU peripheral and sees current settings translated into
...
**Independent Test**: Load a known MK3 configuration fixture and verify that representative clock,
GPIO, timer, ADC, DMA, UART, SPI, and I2C settings are described correctly and traceable to their
...
- Multiple ST-Link devices or MK3 targets are attached.
...
- **FR-001**: The system MUST discover and open an explicitly selected supported MK3 through the
  existing read-only programmer session and MUST never auto-select ambiguously among multiple
  targets.
...
- **FR-007**: The system MUST decode at least the clock tree, GPIO, timers, ADC, DMA, UART, SPI, and
  I2C configuration used by the current MK3 firmware, with a generic evidence view for other mapped
  peripherals.
...
- **FR-010**: The system MUST support search and filtering by MK3 subsystem, peripheral, register,
...
- **Project Snapshot**: A coherent, versioned set of MK3 state and configuration values published by
  the running target for observation.
...
- **SC-001**: An engineer sees the first coherent MK3 dashboard within three seconds of granting
...
- **SC-004**: All supported MK3 clock, GPIO, timer, ADC, DMA, UART, SPI, and I2C fixture settings are
  decoded correctly and remain traceable to raw evidence in automated acceptance tests.
...
- **SC-006**: A 30-minute hardware observation produces zero target write, erase, reset, halt, run,
  or step requests and does not interrupt the running MK3 application.
...
- The MK3 firmware will expose a deliberately designed, coherent debug snapshot in readable SRAM;
  arbitrary application variables will not be discovered heuristically.
...
- Existing uncommitted MK3 simulator and board-viewer work is preserved and is not a dependency for
